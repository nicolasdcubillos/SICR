...
@Service
public class EstadoTransferenciaItemService {

    Logger logger = LoggerFactory.getLogger(EstadoTransferenciaItemService.class);

    @Autowired
    private EstadoTransferenciaItemRepository repository;

    public RespuestaServicioVO getById(Integer id) {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        try {
            EstadoTransferenciaItem estadoTransferenciaItem = repository.findById(id).get();
            respuesta.setObjeto(estadoTransferenciaItem);
    ...
    public RespuestaServicioVO getAll() {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        try {
            Iterable<EstadoTransferenciaItem> coldatos = repository.findAll();
    ...
    public RespuestaServicioVO getByPage(Integer numeroPagina, Integer registrosPorPagina, String ordenarPor,
        ...
        try {
            ...
            Iterable<EstadoTransferenciaItem> page = repository.getByPage(pageable);
    ...
    public RespuestaServicioVO crear(EstadoTransferenciaItem estadoTransferenciaItem) {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        try {
            respuesta = crearTX(estadoTransferenciaItem);
        } catch (DataAccessException e) {
            respuesta.setObjeto(null);
            respuesta.setExitosa(false);
    ...
    public RespuestaServicioVO crearTX(EstadoTransferenciaItem estadoTransferenciaItem) {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        repository.save(estadoTransferenciaItem);
        respuesta.setObjeto(estadoTransferenciaItem);
        respuesta.setExitosa(true);
        return respuesta;
    }

    public RespuestaServicioVO actualizar(EstadoTransferenciaItem estadoTransferenciaItem) {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        try {
            respuesta = actualizarTX(estadoTransferenciaItem);
        } catch (DataAccessException e) {
            respuesta.setObjeto(null);
            respuesta.setExitosa(false);
            respuesta.setDescripcionExcepcion(e.getMessage() + " - " + e.getRootCause().getMessage());
    ...
    @Transactional(rollbackFor = Exception.class)
    public RespuestaServicioVO actualizarTX(EstadoTransferenciaItem estadoTransferenciaItem) {
        RespuestaServicioVO respuesta = new RespuestaServicioVO();
        EstadoTransferenciaItem aux = null;
        aux = repository.findById(estadoTransferenciaItem.getId()).get();
        if (aux != null) {
            estadoTransferenciaItem.setId(aux.getId());
            repository.save(estadoTransferenciaItem);
            respuesta.setObjeto(estadoTransferenciaItem);
            respuesta.setExitosa(true);
